...
urls.forEach(url => {
  describe(`Login Page UI Elements Test on ${url}`, () => {
    ...
    it("should validate login page title and meta data", () => {
      ...
      cy.document()
        ...
        .should("have.attr", "content", "ACME demo app");
      cy.document()
        .get('head meta[name="keywords"]')
        .should("have.attr", "content", "Applitools Visual testing");
    });
    ...
    it("should validate username component", () => {
      cy.get("form").within(() => {
        ...
        cy.get(".pre-icon:first").should(
          "have.class",
          "pre-icon os-icon os-icon-user-male-circle"
        );
    ...
    it("should validate password component", () => {
      cy.get("form").within(() => {
        ...
        cy.get(".pre-icon")
          .eq(1)
          .should("have.class", "pre-icon os-icon os-icon-fingerprint");
    ...
    it("should have proper social media links", () => {
      cy.get(".buttons-w a")
        .eq(0)
        .should("include.html", 'src="img/social-icons/twitter.png"');
      cy.get(".buttons-w a")
        .eq(1)
        .should("include.html", 'src="img/social-icons/facebook.png"');
      cy.get(".buttons-w a")
        .eq(2)
        .should("include.html", 'src="img/social-icons/linkedin.png"');
    });
  });
  ...
  describe(`Data-Driven Test - Login  on ${url}`, () => {
    ...
    it("should show proper messaging when logging in with no username or password", () => {
      cy.get("#log-in").click();
      cy.get('div[id*="random_id_"]').should(
        "include.text",
    ...
    it("should allow login with any username and password combination", () => {
      ...
      cy.get("form #password").type("Passymcpassword!1");
      cy.get("#log-in").click();
      cy.url().should("contain", "hackathonApp");
      cy.get(".top-menu-controls > .logged-user-w").should(
        "include.html",
        'src="img/avatar1.jpg"'
      );
    });
  ...
  describe(`Table Sort Test - Recent Transactions on ${url}`, () => {
    ...
    it("should sort Recent Transaction by Amounts in ascending order", () => {
      cy.login();
      cy.get("#amount").click();
      cy.get("#transactionsTable>tbody>tr>.text-right:first").should(
        "contain.text",
        "- 320.00 USD"
      );
      cy.get("#transactionsTable>tbody>tr>.text-right:last").should(
        "contain.text",
        "+ 1,250.00 USD"
      );
    });
